...
function findTagIdsByNames(db, tagNames) {
  ...
  const idByName = new Map(
    rows.map((row) => [String(row.name).toLowerCase(), row.id]),
  );
...
async function suggestVideoLocation(video, destinations) {
  ...
  const prompt = [
    ...
    "Use the private notes/context as supporting information when it helps fill in missing pieces, but do not ignore stronger evidence from the title, description, or known destinations.",
